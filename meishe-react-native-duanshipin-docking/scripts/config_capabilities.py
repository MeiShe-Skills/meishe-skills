...
def _apply_instructions(track: str, platform: str) -> list[str]:
    if track not in TRACKS:
        raise CapabilityCatalogError(f"Unsupported React Native track `{track}`")
    run_command = (
        "npm run android -- --deviceId <ANDROID_DEVICE_ID>"
        if platform == "android"
        else "npm run ios -- --udid <IOS_DEVICE_UDID>"
    )
    return ["npm start", run_command, "Metro 已运行时按 r 重新加载；原生打包输入变化时重新构建"]
...
def _suggested_mutation(
    ...
    member = (
        values[0]["member"]
        if values
        else fields[0]["member"]
        if fields
        else capability["members"][0]
    )
    ...
    *,
    ...
    for capability in catalog["capabilities"]:
        ...
        if concrete_hits:
            ...
            resolved = dict(capability)
        if score:
            resolved["suggestedMutation"] = _suggested_mutation(
                catalog,
                resolved,
                platform,
            )
