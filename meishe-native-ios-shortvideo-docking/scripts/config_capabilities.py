...
def _apply_instructions(track: str, platform: str) -> list[str]:
    if track not in TRACKS or platform != "ios":
        raise CapabilityCatalogError(f"Unsupported native iOS route `{track}/{platform}`")
    return [
        "open <IOS_WORKSPACE_ABSOLUTE_PATH>",
        "在 Xcode 选择实际 App scheme 和已连接的真实 iPhone/iPad 后执行 Product > Run（Command-R）",
    ]
