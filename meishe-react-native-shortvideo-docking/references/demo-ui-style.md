...
- React Native wrappers must use bounded waits for `configureServer()` and `downloadPrefabricatedMaterial()` because the official plugin Promise can remain unresolved if the native SDK never calls back or the network is unavailable. Prefer short timeouts such as 6 seconds for server configuration and 12 seconds for prefabricated material download.
...
- React Native generated templates must copy and reference the bundled banner and icon assets automatically.
