...
6. 重新运行时提供包含 flutter/nvshortvideo 的解压根目录，或直接提供 nvshortvideo 插件目录：
   --plugin-path <extracted-package-root-or-nvshortvideo-folder>

未传 --plugin-path 时，自动查找范围仅限 --target-root 当前项目目录。如果当前项目目录内没有 nvshortvideo，脚本会停止并要求你提供路径，不会在全局范围内查找。
传入 --plugin-path 时，脚本只检查你提供的路径，找到真实 nvshortvideo 文件夹，并复制到目标项目 vendor/meishe/nvshortvideo。
