...
def validate_skill_structure(
    *,
    ...
    dependency_expectations = {
        ...
        "flutter": ("flutter devices", "flutter run -d <ANDROID_DEVICE_ID>", "flutter run -d <IOS_DEVICE_ID>", "Product > Run"),
    ...
    if "ios" in platforms:
        assert_contains(
            dependency_text,
            ["推荐使用 Xcode", "命令行运行方式也必须完整提供"],
            "recommended Xcode and required command-line policy",
        )
        assert_not_contains(
            dependency_text,
            ["命令行备选", "命令行可选"],
            "command-line run wording",
        )
        assert_contains(
            skill_text,
            [
    ...
    forbidden_core_terms = {
        "native-android": ("def add_cocoapods_dependency_step(", "def detect_node_install_command(", "class TargetPlatforms", "iOS Quick Verify", "RN iOS"),
        "native-ios": ("def add_android_gradle_dependency_step(", "def detect_node_install_command(", "class TargetPlatforms"),
        "react-native": ("FIXED_PLATFORM = \"flutter\"",),
        "flutter": ("def detect_node_install_command(", "FIXED_PLATFORM = \"react-native\""),
    }
    assert_not_contains(core_text, forbidden_core_terms[capability_track], "route-specific shared core")
