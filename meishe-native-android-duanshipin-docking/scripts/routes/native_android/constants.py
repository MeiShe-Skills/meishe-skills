...
项目创建完成后，可以将 AAR 放到目标 app module 的 app/libs/NvShortVideoCore.aar，
或重新运行并传入 --aar-path <path-to-NvShortVideoCore.aar>。脚本会把它复制为项目内 app/libs/NvShortVideoCore.aar，
...
6. 将 AAR 放到目标 app module 的 app/libs/NvShortVideoCore.aar，或重新运行并传入 --aar-path <path-to-NvShortVideoCore.aar>。
7. 脚本会把 AAR 复制为项目内 app/libs/NvShortVideoCore.aar，Gradle 不会引用外部下载/解压目录。
