...
def validate_catalog(catalog: dict[str, Any], *, expected_track: str | None = None) -> None:
    ...
    integration_versions = catalog["integrationVerifiedVersions"]
    if not isinstance(integration_versions, dict) or not integration_versions:
        raise CapabilityCatalogError(f"{track}: integrationVerifiedVersions must be a non-empty object")
    for version, state in integration_versions.items():
        if not isinstance(version, str) or not version:
            raise CapabilityCatalogError(f"{track}: integration version keys must be non-empty strings")
        if not isinstance(state, dict) or set(state) != {"status", "evidence", "behavior"}:
            raise CapabilityCatalogError(
                f"{track}/{version}: integration state must contain status, evidence, and behavior"
            )
        if state["status"] != "verified":
            raise CapabilityCatalogError(f"{track}/{version}: integration status must be verified")
        if state["evidence"] not in EVIDENCE_LEVELS:
            raise CapabilityCatalogError(f"{track}/{version}: invalid integration evidence level")
        if not isinstance(state["behavior"], str) or not state["behavior"]:
            raise CapabilityCatalogError(f"{track}/{version}: integration behavior is required")
...
def _apply_instructions(track: str, platform: str) -> list[str]:
    if track not in TRACKS or platform != "android":
        raise CapabilityCatalogError(f"Unsupported native Android route `{track}/{platform}`")
    return [
        "cd <ANDROID_PROJECT_ABSOLUTE_PATH>",
        "./gradlew :app:assembleDebug",
        "在 Android Studio 选择已连接的真实 Android 设备并点击 Run 重新安装",
    ]
