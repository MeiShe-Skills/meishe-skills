...
def main() -> int:
    ...
    if not matches:
        if args.version and args.version not in catalog["verifiedVersions"]:
            integration_state = catalog["integrationVerifiedVersions"].get(args.version)
            if integration_state:
                print(
                    f"`{args.track}` `{args.platform}` version `{args.version}` has verified integration "
                    f"compatibility ({integration_state['evidence']}): {integration_state['behavior']} "
                    "However, the requested configuration behavior is not verified for this version. "
                    "Keep the field status unverified, inspect the selected AAR API, and do not inherit "
                    "a configuration patch from another version.",
                    file=sys.stderr,
                )
                return 1
            print(
